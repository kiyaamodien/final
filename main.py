...
def delete_room(room_id):
    ...
    with sqlite3.connect("users.db") as conn:
        cursor = conn.cursor()
        cursor.execute("DELETE FROM room WHERE id=" + str(room_id))
        conn.commit()
        response['status_code'] = 200
    ...
    if request.method == "PUT":
        with sqlite3.connect('users.db') as conn:
            ...
            if incoming_data.get("room_name") is not None:
                ...
                with sqlite3.connect('users.db') as conn:
                    cursor = conn.cursor()
                    cursor.execute("UPDATE shop SET room_name =? WHERE id=?", (put_data["product_name"], product_id))
                    conn.commit()
                    response['message'] = "Update was successfully added"
                    response['status_code'] = 200
                return response
            if incoming_data.get("room_type") is not None:
                put_data['room_type'] = incoming_data.get('room_type')

                with sqlite3.connect('users.db') as conn:
                    cursor = conn.cursor()
                    cursor.execute("UPDATE shop SET room_type =? WHERE id=?", (put_data["product_type"], product_id))
                    conn.commit()
